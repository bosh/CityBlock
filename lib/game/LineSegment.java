...
public class LineSegment {
	...
	public int distanceTo(LineSegment other){ //Only returns parallel line distances
		int distance = 999;
		if ((this.x1 == this.x2) && (other.x1 == other.x2)) {	//If the segments are vertically parallel
			if ((this.x2 < other.x1)||(this.x1 > other.x2)){	//any of the Xs are between the Xs of the other shape
				distance = Math.abs((int)(this.x1 - other.x1));	//simple difference (as though parallel lines)
			}
			...
			if ((this.y2 < other.y1)||(this.y1 > other.y2)){			//any of the Ys are between the Ys of the other shape
				distance = Math.abs((int)(this.y1 - other.y1));			//simple difference (as though parallel lines)
			}
		}
		return distance;
	}
